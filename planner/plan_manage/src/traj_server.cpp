...
{
    if(last_cmd_state_.pos.norm() > 1e-6 || last_cmd_state_.vel.norm() > 1e-6 || last_cmd_state_.acc.norm() > 1e-6)
    {
        DroneState state;
        ...
        publishCmd(state);
    }else if(odom_state_.pos.norm() > 1e-6 || odom_state_.vel.norm() > 1e-6)
    {
        DroneState state;
